...
describe("groups secure flow", () => {
  ...
  afterEach(function () {
    const testName = this.currentTest?.fullTitle() || "groups secure flow"
  ...
  it("runs create→join→chat flow in secure mode with strict relay preconditions", () => {
    visitWithSecureFixture("/groups/create")
    waitForAppReady()
    ...
    cy.get("body", {timeout: 20000}).then(($body: JQuery<HTMLElement>) => {
      ...
      cy.get("input[placeholder='Group address']", {timeout: 20000})
        .clear()
        .type("relay.navcom.app'opssecure")
      cy.get(
        'textarea[placeholder="One relay per line (invite relay will prefill when available)"]',
      )
        .first()
      ...
      cy.visit("/groups/relay.navcom.app'opssecure/chat")
      waitForAppReady()
      ...
      cy.window()
        ...
        .should((events: Array<{event: string; props: Record<string, unknown>}>) => {
          ...
          expect(createAttempt.props.privacy).to.eq("secure")
