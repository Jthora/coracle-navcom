...
describe("groups max flow", () => {
  ...
  afterEach(function () {
    const testName = this.currentTest?.fullTitle() || "groups max flow"
  ...
  it("runs create→join→chat flow in max mode with strict relay preconditions", () => {
    visitWithMaxFixture("/groups/create")
    waitForAppReady()
    ...
    cy.get("body", {timeout: 20000}).then(($body: JQuery<HTMLElement>) => {
      ...
      cy.get("select", {timeout: 20000}).select("max")
      cy.get('input[placeholder="Room name (e.g. ops)"]', {timeout: 20000}).clear().type("opsmax")
      cy.get(
      ...
      cy.contains("Max Diagnostics", {timeout: 20000})
      cy.contains("Max ready", {timeout: 20000})
      ...
      cy.visit("/groups/relay.navcom.app'opsmax/chat")
      waitForAppReady()
      ...
      cy.window()
        ...
        .should((events: Array<{event: string; props: Record<string, unknown>}>) => {
          ...
          expect(createAttempt.props.requested_transport_mode).to.eq("secure-pilot")

          const joinAttempt = expectEvent(events, "group_setup_join_attempt")
          expect(joinAttempt.props.requested_transport_mode).to.eq("secure-pilot")

          const securityShown = expectEvent(events, "group_security_state_shown")
          expect(
            String(securityShown.props.state || securityShown.props.security_state).length,
          ).to.be.greaterThan(0)
        })
    })
  })
