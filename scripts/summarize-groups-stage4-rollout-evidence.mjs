...
const outputJson = resolve(
  process.cwd(),
  getArgValue("--output-json", "docs/groups/upgrade/cache/stage4-rollout-evidence-summary.json"),
)
const outputMarkdown = resolve(
  process.cwd(),
  getArgValue("--output-markdown", "docs/groups/upgrade/cache/stage4-rollout-evidence-summary.md"),
)
...
const rows = buildSummaryRows(STAGE4_SUBTASK_ROWS, entries)
...
const markdown = [
  "# Stage 4 Rollout Evidence Summary",
  "",
...
process.stdout.write(`STAGE4_ROLLOUT_SUMMARY_JSON:${outputJson}\n`)
process.stdout.write(`STAGE4_ROLLOUT_SUMMARY_MD:${outputMarkdown}\n`)
