...
const START = "<!-- GROUPS_STAGE4_SUMMARY_START -->"
const END = "<!-- GROUPS_STAGE4_SUMMARY_END -->"
...
const summaryPath = resolve(
  process.cwd(),
  getArgValue("--summary", "docs/groups/upgrade/cache/stage4-rollout-evidence-summary.json"),
)
...
const previewPending = pendingTaskKeys.slice(0, 8)
...
const generatedBlock = [
  ...
  ...(previewPending.length > 0
    ? ["- Pending Task Keys (up to 8):", ...previewPending.map(taskKey => `  - ${taskKey}`)]
    : ["- Pending Task Keys (up to 8): none"]),
  END,
  "",
].join("\n")
