...
const executionLog = readFileSync(logPath, "utf8")
const entries = parseExecutionEntries(executionLog)
const completionById = buildCompletionMap(STAGE3_SUBTASK_ROWS, STAGE3_AGGREGATE_RULES, entries)

const idPrefix = "S3-P2-ST1"
...
const next = current
  ...
  .map(line => {
    ...
    if (!id?.startsWith(idPrefix) && id !== "S3-P2") {
      return line
    }
