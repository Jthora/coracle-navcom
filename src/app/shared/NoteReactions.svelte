...
<div class="text-sm">
  {#if $zaps.length > 0}
    ...
    {#if pubkeys.length === 1}
      ...
      <p class="flex shrink-0 items-center gap-1 text-nc-text">
        <Icon icon="bolt" />
        <strong>{zapsTotal}</strong> sats from
        <PersonLink {pubkey} />
        <span class="flex items-center gap-1 rounded-full bg-nc-input-l px-2 py-1 text-sm">
          <NoteContentKind7 note={$zaps[0].request} />
        </span>
      </p>
    {:else}
      <div class="flex flex-wrap items-center gap-1 text-nc-text">
        ...
        {#each groupBy(z => z.request.content, $zaps) as [content, zapEvents] (content)}
          ...
          <Link
            ...
            class="flex items-center gap-1 rounded-full bg-nc-input-l px-2 py-1 text-sm">
  ...
  {#if reposts.length > 0}
    ...
    <p class="mt-4 flex items-center gap-1 text-nc-text first:mt-0">
  ...
  {#if reactions.length > 0}
    {#if reactions.length === 1}
      <p class="mt-4 flex shrink-0 flex-wrap items-center gap-1 text-nc-text first:mt-0">
        <PersonLink pubkey={reactions[0].pubkey} />
        reacted with
        <NoteContentKind7 note={reactions[0]} />
      </p>
    {:else}
      <p class="mt-4 flex flex-wrap items-center gap-1 text-nc-text first:mt-0">
